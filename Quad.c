...
Quad *new_Quad(int tx, int ty, int w, int h, int wsplit, int sx) {
    ...
    if (tx > sx || tx + w > sx) return NULL;
    if (wsplit != 1 && wsplit != 0) return NULL;
...
int similar(Image *im, Quad *q, int threshold) {
    ...
    int maxCol = -1, minCol = 256;
    
    for (int y = q->ty; y < (q->ty + q->h); y++) {
        for (int x = q->tx; x < (q->tx + q->w); x++) {
            if (im->data[(x + (y * im->sx))] > maxCol) {
                maxCol = im->data[x + (y * im->sx)];
            }
            else if (im->data[x + (y * im->sx)] < minCol) {
                minCol = im->data[x + (y * im->sx)];
            }
        }
    }
...
///////////////////////////////////////////////////////////////////////////////
Quad *split_tree_helper(Image *im, Quad *root, int threshold, Quad* newRoot) {
    
    if (root == NULL) return NULL;
        
        if (root->left != NULL) newRoot = split_tree_helper(im, root->left, threshold, newRoot);
        if (root->right != NULL) newRoot = split_tree_helper(im, root->right, threshold, newRoot);
        
        // check if root is similar
        if (similar(im, root, threshold) == 0) {
            
            // check wsplit
            if (root->wsplit == 1) {
                newRoot = BST_insert(newRoot, new_Quad(root->tx, root->ty, root->w/2, root->h, 0, root->sx));
                newRoot = BST_insert(newRoot, new_Quad(root->tx + root->w/2, root->ty, root->w/2,  root->h, 0, root->sx));
            }
            else {
                newRoot = BST_insert(newRoot, new_Quad(root->tx, root->ty, root->w, root->h/2, 1, root->sx));
                newRoot = BST_insert(newRoot, new_Quad(root->tx, root->ty + root->h/2,  root->w,  root->h/2, 1, root->sx));
            }
        }
        else if (similar(im, root, threshold) == 1) {
            newRoot = BST_insert(newRoot, new_Quad(root->tx, root->ty, root->w, root->h, root->wsplit, root->sx));
        }
...
Quad *split_tree(Image *im, Quad *root, int threshold) {
    ...
    Quad *newRoot = NULL;
    newRoot = split_tree_helper(im, root, threshold, newRoot);
    delete_BST(root);
